...
def segment_vietnamese(text: str) -> str:
    ...
    try:
        ...
        # underthesea nối từ ghép bằng "_" (vd "nghỉ_phép"). Replace để query
        # 2-token "nghỉ phép" khớp được với BM25 (split theo space).
        return segmented.replace("_", " ")
    except Exception:
        return text  # fallback nếu underthesea chưa cài / lỗi
...
class DenseSearch:
    def __init__(self):
        ...
        # Thử Qdrant server (docker). Nếu không chạy được (vd không pull được
        # image do mạng), fallback sang Qdrant embedded lưu trên đĩa — không cần
        # Docker. Hai mode dùng cùng API nên phần còn lại của code không đổi.
        try:
            client = QdrantClient(host=QDRANT_HOST, port=QDRANT_PORT, timeout=3)
            client.get_collections()  # probe kết nối
            self.client = client
            print("  [DenseSearch] dùng Qdrant server localhost:6333")
        except Exception:
            # In-memory: né việc pull image Docker và tránh tranh lock khi
            # main.py tạo nhiều DenseSearch trong cùng 1 process (naive + prod).
            self.client = QdrantClient(location=":memory:")
            print("  [DenseSearch] Qdrant server không sẵn sàng → embedded in-memory")
        self._encoder = None
